import { httpAddOrUpdateMeasure } from '@/services';
import { Button, Form, Input, Switch, message } from 'antd';
import { useState } from 'react';

export default function MeasureFrom({ measure, hidden, onClose }) {
    const [loading, setLoading] = useState(false);

    const handleOnSubmit = values => {
        setLoading(true);
        httpAddOrUpdateMeasure({
            ...measure,
            ...values,
    ...
    return (
        <Form
            layout='vertical'
            initialValues={{ ...measure, estado: !measure?.estado || measure?.estado === '1' ? true : false }}
            onFinish={handleOnSubmit}
        >
            <Form.Item name='medida' label='Nombre' rules={[{ required: true, message: 'El campo es obligatorio' }]}>
